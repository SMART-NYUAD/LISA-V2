#!/usr/bin/env python3
"""
LISA - AI-Powered Robot Navigation & Safety Assistant (Local + Keyboard Mode)

This script runs LISA with a local Ollama instance and keyboard text input.
Suitable for development, testing, and deployment without network dependencies.

Key Features:
- Local Ollama for LLM inference (low latency)
...
Environment Variables:
    LISA_CHAT_MODEL: Chat model name (default: gemma3:4b)
    LISA_VISION_MODEL: Vision model name (default: gemma3:4b)
...
def main():
    """Main function to run the local navigation and camera assistant."""
    chat_model = DEFAULT_CHAT_MODEL
    vision_model = DEFAULT_VISION_MODEL
    
    # Tee terminal output to file
    setup_terminal_log("../logs/local")
    ...
    # Welcome message
    print("\n=== LISA - Navigation & Camera Assistant (V2 Local) ===")
    print(f"Using local Ollama chat model: {chat_model}")
    print(f"Using local Ollama vision model: {vision_model}")
        ...
        try:
            ...
            if function_call:
                ...
                while True:
                    ...
                    next_response = chat_with_local_ollama(
                        model=chat_model,
                        messages=messages,
                        options={'temperature': 0.3}
                    )
